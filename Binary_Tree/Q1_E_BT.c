...
{
    // Note that two binary trees are structurally identical if they are both empty or if they are both non-empty and the left and the right subtrees are similar

    // 1. 두 노드가 전부 null -> return 1
    if (tree1 == NULL && tree2 == NULL) // node가 null인것과 item이 null인것의 차이?
        return 1;
    // 2. 두 노드 중 하나가 null -> return 0
    else if (tree1 == NULL || tree2 == NULL)
        return 0;
    // 3  두 노드 값 다름 -> return 0
    else if (tree1->item != tree2->item)
        return 0;
    // 4. 두 노드 값이 같음
    else
        identical(tree1->left, tree2->left) && identical(tree1->right, tree2->right);
}
