...
void levelOrderTraversal(BSTNode *root)
{
	...
	{
		BSTNode *queueNode = dequeue(&(temp_queue.head), &(temp_queue.tail));
		printf("%d, ", queueNode->item);

		if (queueNode->left != NULL) // 예외처리 추가-----> queueNode->left가 null인 경우
		{
			enqueue(&(temp_queue.head), &(temp_queue.tail), queueNode->left);
		}
		if (queueNode->right != NULL) // 예외처리 추가-----> queueNode->right가 null인 경우
		{
			enqueue(&(temp_queue.head), &(temp_queue.tail), queueNode->right);
		}
	}
}
